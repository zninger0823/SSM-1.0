...
public class ProductDaoTest extends BaseTest{
	...
	public void testBQueryProductList() throws Exception {
		...
		//查询名称为测试商品的总数
		int count = productDao.queryProductCount(productConditon);
		assertEquals(6, count);
		//使用商品名称模糊查询，预期返回两条结果
		productConditon.setProductName("测试");
		productList = productDao.queryProductList(productConditon, 0, 3);
		assertEquals(3, productList.size());
		count = productDao.queryProductCount(productConditon);
...
//		assertEquals(1, count);
	...
	public void testEUpdateProductCategoryToNull(){
		//将productCategory为5的商品类别下面的商品的商品的商品类别置为空
	...
	public void testDUpdateProduct() throws Exception {
		...
		Shop shop=new Shop();
		shop.setShopId(5L);
		pc.setProductCategoryId(22L);
		
		product.setProductId(5L);
		product.setShop(shop);
		product.setProductName("第11个产品");
